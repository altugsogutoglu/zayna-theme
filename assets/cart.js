...
(() => {
    ...
    document.querySelectorAll('[data-cart-money-root]').forEach((root) => {
      ...
      if (message) {
        if (remainingCents === 0) {
          ...
          message.innerHTML =
            'Nog <span data-free-shipping-remaining>' +
  ...
  function setClearPanel(clearRoot, expanded) {
    if (!clearRoot) return;

    const toggle = clearRoot.querySelector('[data-cart-clear-toggle]');
    const panel = clearRoot.querySelector('[data-cart-clear-panel]');
  ...
  function collectClearUpdates(root) {
    const updates = {};

    (root || document)
      .querySelectorAll('[data-cart-line][data-line-key]')
      .forEach((line) => {
  ...
  async function clearCart(trigger) {
    if (actionBusy) return;

    const root =
      trigger?.closest('[data-cart-money-root]') ||
      getPrimaryCartRoot();
    const updates = collectClearUpdates(root);
    ...
    const viewState = captureCartViewState({
      anchorElement: root?.querySelector('[data-cart-lines]') || null,
      suppressFocus: true,
    });
    ...
    try {
      ...
      applySections(data.sections, {
        ...
        page: viewState?.page || null,
      });
    } catch (error) {
      console.error('Zayna cart clear failed:', error);

      try {
        await refreshCartFromServer(viewState);
      } catch (refreshError) {
        console.error(
          'Zayna cart refresh after clear failed:',
          refreshError
        );
      }
      ...
      if (button?.isConnected) {
        button.disabled = false;
        if (button.textContent === 'Leegmaken…') {
  ...
  document.addEventListener('click', (event) => {
    ...
    const clearToggle = event.target.closest(
      '[data-cart-clear-toggle]'
    );

    if (clearToggle) {
      const clearRoot = clearToggle.closest('[data-cart-clear]');
      const expanded =
        clearToggle.getAttribute('aria-expanded') === 'true';

      setClearPanel(clearRoot, !expanded);
      return;
    }

    const clearCancel = event.target.closest(
      '[data-cart-clear-cancel]'
    );

    if (clearCancel) {
      setClearPanel(clearCancel.closest('[data-cart-clear]'), false);
      clearCancel
        .closest('[data-cart-clear]')
        ?.querySelector('[data-cart-clear-toggle]')
        ?.focus({ preventScroll: true });
      return;
    }

    const clearConfirm = event.target.closest(
      '[data-cart-clear-confirm]'
    );

    if (clearConfirm) {
      clearCart(clearConfirm);
    }
  });
  ...
  document.addEventListener('submit', (event) => {
    ...
    if (checkoutForm) {
      ...
      if (
        checkoutForm.dataset.cartCheckoutBusy === 'true'
      ) {
        return;
      }
      ...
      const originalLabel =
        checkoutButton.textContent.trim() || 'Afrekenen';
      ...
      settleQuantityUpdates()
        ...
        .catch((error) => {
          ...
          delete checkoutForm.dataset.cartCheckoutBusy;
          checkoutButton.disabled = false;
