...
4b. **Full-page mixed pagination (post-review note):** `search.results` is a single relevance-ordered array of mixed `object_type`; Liquid's `{% paginate %}` cannot take a `where`-filtered (product-only) subset, so it pages the mixed list and the product grid renders only the products within each slice. Products are never lost or duplicated (the loader appends only `[data-product-grid]` cells and pages/articles render once from the un-paginated set), but the per-page product count can vary slightly (mostly page 1, where the handful of page/article hits live). Accepted as an inherent platform limitation — folded into the divergence doc (Task 9). The product-section guard uses `search.results | where: 'object_type', 'product'` (`.size`) rather than a `for`-counter, which would hit Liquid's 50-iteration loop cap and could falsely hide products.
